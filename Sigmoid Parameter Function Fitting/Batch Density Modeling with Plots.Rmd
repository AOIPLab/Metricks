...
This script requires 2-3 input csv files depending on comparison mode:

1. Data input file(s) (e.g., Density_Extractions_Radial.csv)
...
orientation_type <- tolower(readline(prompt = "Enter orientation type: horizontal/vertical/radial"))

# Prompt for first input file
cat("\nNote: CSV should be in PAIRED COLUMN format.\n")
cat("  Row 1 = Subject IDs (repeated for each E,D pair)\n")
cat("  Rows 2+ = Paired columns (E1,D1,E2,D2,...)\n")
input_file_1 <- readline(prompt = "Enter first input CSV filename (e.g., Density_Extractions_Radial.csv): ")
...
if (comparison_mode) {
  input_file_2 <- readline(prompt = "Enter second input CSV filename (e.g., control.csv): ")
...
# Load parameter ranges from parameter_inputs.csv (assumed to be in same folder as script)
...
This first part loads horizontal (or however specified) density data and fits a nonlinear least squares regression model on them to define optimal parameters.
...
# Function to parse paired column format
...
# ---- Create plots folder ----
if (group_1_name == "") {
  plots_dir_1 <- paste(orientation_type, "Plots", sep = " ")
} else {
  plots_dir_1 <- paste0(group_1_name, orientation_type, " Plots")
}
...
master_df_nls <- lapply(split(long_df, long_df$Subject), function(x) {
  ...
  if(!all(is.na(params))) {
    ...
    ggsave(file.path(plots_dir_1, paste0("fit_plot_", safe_subject, "_", orientation_type, ".png")), gg,
...
file_prefix_1 <- if (group_1_name == "") orientation_type else paste0(group_1_name, "_", orientation_type)
...
plot_title_1 <- if (group_1_name == "") {
  paste0("Density Plots for ", tools::toTitleCase(orientation_type), " Parameters")
} else {
  paste0("Density Plots for ", tools::toTitleCase(orientation_type), " ", group_1_name, " Parameters")
}
...
master_df_nls <- lapply(split(long_df, long_df$Subject), function(x) {
  ...
  if(!all(is.na(params))) {
    ...
    safe_subject <- gsub("[^A-Za-z0-9_]", "_", unique(x$Subject))
    ggsave(file.path(plots_dir_2, paste0("fit_plot_", safe_subject, "_", orientation_type, ".png")), gg,
           width = 6, height = 4, dpi = 300)
  }
...
# ---- Write results (to plots folder) ----
write.csv(master_df_nls, file.path(plots_dir_2, paste0(group_2_name, "_", orientation_type, "_parameter_output_NLS", version_suffix, ".csv")), row.names = FALSE)
...
write.csv(summary_stats, file.path(plots_dir_2, paste0(group_2_name, "_", orientation_type, "_parameter_summary_NLS", version_suffix, ".csv")), row.names = FALSE)
...
p_group_2 <- ggplot(parameters_group_2, aes(x = Value)) +
  ...
  facet_wrap(~ Parameter, scales = "free") +
  theme_bw() +
  labs(title = paste0("Density Plots for ", tools::toTitleCase(orientation_type), " ", group_2_name, " Parameters"), x = "Value", y = "Density")

ggsave(
  filename = file.path(plots_dir_2, paste0(group_2_name, "_", orientation_type, "_parameter_densities.png")),
  plot = p_group_2,
  width = 10,
  height = 8,
  dpi = 300
...
Now export plits and potentially run comparisons
...
### --- Save workbook ---
saveWorkbook(wb, file = paste0("Parameter Summaries/", tools::toTitleCase(orientation_type), "_Decay_Parameter_Summaries", version_suffix, ".xlsx"), overwrite = TRUE)
```
...
ggsave(
  filename = file.path(plots_dir, paste0(group_2_name, "_", orientation_type, "_parameter_densities.png")),
  plot = p_group_2,
  width = 10,
  height = 8,
  dpi = 300)
ggsave(
  filename = file.path(plots_dir, paste0(group_1_name, "_", orientation_type, "_parameter_densities.png")),
  plot = p_group_1,
  width = 10,
  height = 8,
  dpi = 300)
...
combined_violin <- ggplot(df_long, aes(x = Dataset, y = Value, fill = Dataset)) +
  ...
  theme_minimal() +
  labs(
    title = paste0("Violin Plots of ", tools::toTitleCase(orientation_type), " Density Parameters by Dataset"),
    x = "Dataset",
    y = "Value"
  ) +
...
ggsave(
  filename = file.path(plots_dir, paste0("violin_", orientation_type, "_all_parameters.png")),
...
for (var in vars_to_plot) {
  ...
  p_base <- ggplot(df_var, aes(x = Dataset, y = Value, fill = Dataset)) +
    ...
    theme_minimal(base_size = 14) +
    labs(
      title = paste("Violin Plot of", var, "by Dataset (", tools::toTitleCase(orientation_type), ")"),
      x = "Dataset",
      y = var
    ) +
    scale_fill_manual(values = group_colors) +
  ...
  ggsave(
    filename = file.path(plots_dir, paste0("violin_", orientation_type, "_", var, ".png")),
    plot = p_base,
    width = 6, height = 5,
    dpi = 300
  )
  ...
  ggsave(
    filename = file.path(plots_dir, paste0("scaled_violin_", orientation_type, "_", var, ".png")),
    plot = p_scaled,
    width = 6, height = 5,
    dpi = 300
  )
